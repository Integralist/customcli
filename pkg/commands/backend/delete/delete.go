...
func run(args []string) {
	fmt.Println("backend delete:", args)
}

func Run(args []string) {
	var helpflag bool

	fs := flag.NewFlagSet("backend delete", flag.ExitOnError)
	...
	if helpflag {
		// example := "fastly backend delete --service-id=SERVICE-ID --version=VERSION --name=NAME"
		// description := "Delete a backend on a Fastly service version"
		// print.Help(example, description, "COMMAND FLAGS", cmdFlags)
		os.Exit(1)
	}

	fmt.Println("backend delete logic")
}
